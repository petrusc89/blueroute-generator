...
// Colors
const NAVY = [0, 51, 102] as const;
const TEAL = [0, 196, 140] as const;
const LIGHT_BLUE = [96, 186, 218] as const;
...
export async function generatePdf(inputData: ReportData) {
  ...
  const addFooter = () => {
    ...
    // Red-white-blue line
    const lineY = FOOTER_Y - 3;
    doc.setFillColor(200, 16, 32);
    doc.rect(MARGIN, lineY, 12, 1.5, "F");
    doc.setFillColor(255, 255, 255);
    doc.rect(MARGIN + 12, lineY, 6, 1.5, "F");
    doc.setFillColor(0, 51, 153);
    doc.rect(MARGIN + 18, lineY, 6, 1.5, "F");
    // Line
    doc.setDrawColor(180, 180, 180);
    doc.line(MARGIN + 26, lineY + 0.75, PAGE_W - MARGIN, lineY + 0.75);
    doc.text("Econowind B.V. | www.econowind.nl | +31 (0)6 18 87 00 62", PAGE_W / 2, FOOTER_Y, { align: "center" });
  };
  ...
  const addHeader = () => {
    ...
    doc.text(`Page ${pageNum} / {TOTAL}`, MARGIN, 12);
    doc.text(`${data.version} - ${data.revision}`, PAGE_W / 2, 12, { align: "center" });
    doc.text(`${data.shipName} - ${data.customerName}`, PAGE_W - MARGIN, 12, { align: "right" });
  ...
  doc.setFontSize(18);
  doc.setFont("helvetica", "bold");
  doc.text("ECONOWIND", PAGE_W - MARGIN, 24, { align: "right" });
  doc.setFont("helvetica", "normal");
  ...
  doc.text(subtitle, PAGE_W / 2, titleY + 14, { align: "center" });
  doc.setTextColor(0, 0, 0);
  doc.setFont("helvetica", "normal");

  // Line under subtitle
  doc.setDrawColor(180, 180, 180);
  doc.line(MARGIN + 20, titleY + 18, PAGE_W - MARGIN - 20, titleY + 18);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255, 255, 255);
  doc.text("APPENDIX I", MARGIN + 3, y + 5);
  doc.setTextColor(0, 0, 0);
  doc.text("EEXI WASP FORMULA", MARGIN + 35, y + 5);
  doc.setFont("helvetica", "normal");
  y += 14;
  doc.setFontSize(9);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255, 255, 255);
  doc.text("APPENDIX II", MARGIN + 3, y + 5);
  doc.setTextColor(0, 0, 0);
  doc.text("MEPC.1/CIRC.896", MARGIN + 35, y + 5);
  doc.setFont("helvetica", "normal");
  y += 14;
  doc.setFontSize(9);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255, 255, 255);
  doc.text("APPENDIX III", MARGIN + 3, y + 5);
  doc.setTextColor(0, 0, 0);
  doc.text("ECONOWIND ASSUMPTIONS", MARGIN + 35, y + 5);
  doc.setFont("helvetica", "normal");
  y += 14;
  doc.setFontSize(9);
  const assumptions = [
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255, 255, 255);
  doc.text("APPENDIX IV", MARGIN + 3, y + 5);
  doc.setTextColor(0, 0, 0);
  doc.text("BLUE ROUTE DATA SOURCES AND ASSUMPTIONS", MARGIN + 35, y + 5);
  doc.setFont("helvetica", "normal");
  y += 14;
  doc.setFontSize(8);
  ...
  for (let i = 2; i <= totalPages; i++) {
    doc.setPage(i);
    // Find and replace {TOTAL} placeholder
    // We do this by re-rendering the header
    doc.setFontSize(8);
    doc.setTextColor(255, 255, 255);
    doc.setFillColor(255, 255, 255);
    doc.rect(MARGIN - 1, 9, 30, 5, "F");
    doc.setTextColor(LIGHT_BLUE[0], LIGHT_BLUE[1], LIGHT_BLUE[2]);
    doc.text(`Page ${i} / ${totalPages}`, MARGIN, 12);
  }
  // Page 1 has no header
  doc.setPage(1);
